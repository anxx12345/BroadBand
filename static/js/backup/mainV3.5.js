                ...
                results: function () {
                    this.$nextTick(function () {
                        if(this.results.manufacturer === 'zte'){//如果是中兴OLT
                            ...
                            $singleCatZte = $("#singleCatZte");
                            $doubleCatZte = $("#doubleCatZte");
                            $iptvZte = $('#iptvZte')
                            //判断是单口光猫
                            if(catData.indexOf('eth') !==-1){
                                ...
                                //判断是多口光猫
                                if(online1_lng === 3 && online2_lng === 1 ){
                                    $doubleCatZte.html('<span class="label label-success">多口猫上网数据正常</span>')
                                }else {
                                    $doubleCatZte.html('<span class="label label-danger">多口猫上网数据缺失</span>')
                                }
    ...
    $(document).on('click','#queryQzBtn',function (e) {
        ...
        if(userNum.match(/05570\d{7}/)&&userNum.toString().length === 12){
            ...
            $.post('/oltAdmin',{
                ...
                console.log(result);
                if(result.indexOf('无群障') !== -1){
                    $qunzhangArea.html(`<span class="label label-success">${result}</span>`)
                }else{
                    $qunzhangArea.html(`<span class="label label-danger">${result}</span>`)
                }
            })
        }else {
            $show_area_tip.text('请输入正确的宽带帐号！')
